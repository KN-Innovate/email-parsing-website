...
#     redaction_start = 0
#     current_pii_type = ''
#     local_pii_count = 0  # Count PII redactions
...
#     return ''.join(masked_text), local_pii_count

# def apply_redaction(masked_text, start, end, pii_type, aggregate_redaction):
#     """
#     Applies redaction to the masked_text list from start to end indices.
#     """
...
#             # return email_data, total_pii_count
#         except Exception as e:
...
#     return render_template('parsed-emails.html', emails=parsed_emails, pii_count=pii_count, hotel_name=hotel_name)
...
# #             parsed_emails, pii_count = parse_mbox(filepath)
...
#         if file:
#             # Optional: Sanitize hotel_name to prevent injection or formatting issues
...
#             # Redirect to parsed-emails with hotel_name as query parameter
#             return redirect(url_for('parsed_emails_page', hotel_name=hotel_name))
#         else:
#             return jsonify({"success": False, "message": "No file uploaded."}), 400
#     except Exception as e:
#         return jsonify({"success": False, "message": str(e)}), 500
...
# #     # Calculate PII count for the current page
# #     paginated_pii_count = sum(email['Body'].count('[redacted]') for email in parsed_emails)

# #     # Set up pagination
# #     # from flask_paginate import Pagination, get_page_parameter
# #     # pagination = Pagination(page=page, total=total, per_page=per_page, css_framework='bootstrap5', record_name='emails')

# #     return render_template('parsed-emails.html', 
# #                            emails=parsed_emails, 
# #                            pii_count=paginated_pii_count, 
